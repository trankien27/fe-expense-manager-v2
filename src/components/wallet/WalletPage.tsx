...
const WalletPage: React.FC = () => {
  ...
  const [walletToUpdate, setWalletToUpdate] = useState<Wallet | null>(null);
    ...
    async (page: number = 0, pageSize: number = 10) => {
      try {
        ...
        const response = await axiosInstance.get(
          `/wallets?page=${page}&pageSize=${pageSize}`
        );
        const {
          items,
          currentPage,
  ...
  const handleOpenDelete = (wallet: Wallet) => {
    ...
    setConfirmDeleteChecked(false); // reset checkbox
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      await axiosInstance.delete(`/wallets/${walletToDelete.id}`);
      reloadWallets();
      handleCloseDelete();
      setSuccessMessage('Đã xoá ví thành công!');
      setTimeout(() => setSuccessMessage(''), 3000);
    } catch (err) {
      console.error('Error deleting wallet:', err);
      setError('Không thể xoá ví. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Update
  const handleOpenUpdate = (wallet: Wallet) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto animate-[fadeIn_0.5s_ease-out]">
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={loading}
              className="bg-gray-100 px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-gray-200 transition-all duration-300 hover:shadow-md hover:-translate-y-0.5 disabled:opacity-50"
            >
              <RefreshCw
                size={18}
            ...
            <button
              ...
              className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:shadow-md hover:-translate-y-0.5 transition-all duration-300"
            >
        ...
        {/* Error */}
        {error && (
          <div className="bg-red-50 text-red-700 p-3 rounded mb-4 animate-[slideUp_0.3s_ease-out]">
            {error}
          </div>
        )}
          ...
          <table className="min-w-full">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={4} className="text-center py-6 text-gray-500 animate-[fadeIn_0.5s_ease-out]">
                    Không có ví nào
                  </td>
                </tr>
              ) : (
        ...
        {/* Pagination */}
        <div className="flex justify-center gap-2 mt-6 animate-[fadeIn_0.5s_ease-out]">
          {Array.from({ length: pagination.totalPages }, (_, i) => (
            <button
              key={i}
              onClick={() => handlePageChange(i)}
              className={`px-3 py-1 border rounded transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 ${
                pagination.currentPage === i ? 'bg-blue-600 text-white' : 'bg-white'
              }`}
            >
              {i + 1}
            </button>
      ...
      {isDeleteOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 animate-[fadeIn_0.3s_ease-out]">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md transform transition-all scale-95 animate-[scaleIn_0.3s_ease-out]">
            <h2 className="text-lg font-semibold text-slate-800 mb-4">Xác nhận xoá ví</h2>
            <p className="text-slate-600 mb-4">
              Bạn có chắc chắn muốn xoá ví{' '}
              <span className="font-medium text-slate-800">"{walletToDelete?.name}"</span> không?
            </p>
            ...
            <div className="bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-lg p-3 mb-4 flex items-start gap-2">
              ...
              <span>
                Khi xoá ví này, <strong>tất cả giao dịch liên quan</strong> cũng sẽ bị xoá và không thể khôi phục.
              </span>
            </div>
            ...
            <label className="flex items-center gap-2 text-sm text-slate-700 mb-6">
              ...
              />
              Tôi hiểu rủi ro này và vẫn muốn xoá ví.
            </label>
