  ...
  const fetchRecentTransactions = async () => {
    try {
      const response = await axiosInstance.get<Transaction[]>(
        '/transactions/my-transactions?page=1&pageSize=7'
      );
            ...
            <div key={transaction.id} className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
              <div className="flex items-center space-x-3">
                <div className={`p-2 rounded-full ${transaction.amount < 0 ? 'bg-red-100' : 'bg-green-100'}`}>
                  <Clock className={`h-4 w-4 ${transaction.amount < 0 ? 'text-red-600' : 'text-green-600'}`} />
                </div>
                <div>
                  <p className="font-medium text-gray-900">{transaction.categoryName}</p>
                  <p className="text-sm text-gray-500">{transaction.note}</p>
                  <p className="text-xs text-gray-400">{formatDate(transaction.occurredAt)}</p>
                </div>
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-gray-400">{transaction.walletName}</p>
              </div>
            </div>
          ))}
        </div>
